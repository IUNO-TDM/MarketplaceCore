...
-- 	1) Why is this Patch necessary?
--  Database returns more data than used on marketplacecore. This is a security issue
-- 	2) Which Git Issue Number is this patch solving?
-- #185
-- 	3) Which changes are going to be done?
--  Delete unused return values from functions
--: Run Patches
------------------------------------------------------------------------------------------------
--##############################################################################################
...
$$
	DECLARE
		PatchName varchar		 	 := 'iuno_marketplacecore_V0044V_20180613';
...
DROP FUNCTION public.gettechnologydataforuser(uuid, uuid, text, text[]);
...
CREATE OR REPLACE FUNCTION public.createtechnologydata(
	...
	vcreatedby uuid,
	vroles text[])
    RETURNS TABLE(technologydatauuid uuid, technologydataname character varying, technologyuuid uuid, productcode integer, licensefee bigint, technologydatadescription character varying, technologydatathumbnail bytea, technologydataimgref character varying, backgroundcolor character varying, createdby uuid)
    LANGUAGE 'plpgsql'
...
CREATE OR REPLACE FUNCTION public.gettechnologydatabyid(
	...
	vuseruuid uuid,
	vroles text[])
    RETURNS TABLE(technologydatauuid uuid, technologyuuid uuid, technologydataname character varying, technologydatadescription character varying, productcode integer, licensefee bigint, technologydatathumbnail bytea, technologydataimgref character varying, createdby uuid)
    LANGUAGE 'plpgsql'
...
CREATE OR REPLACE FUNCTION public.gettechnologydatabyofferrequest(
	...
	vuseruuid uuid,
	vroles text[])
    RETURNS TABLE(technologydatauuid uuid, technologyuuid uuid, technologydataname character varying, technologydatadescription character varying, licensefee bigint, technologydatathumbnail bytea, technologydataimgref character varying, backgroundcolor character varying, createdby uuid)
    LANGUAGE 'plpgsql'
...
CREATE OR REPLACE FUNCTION public.settechnologydata(
	...
	vcreatedby uuid,
	vroles text[])
    RETURNS TABLE(technologydatauuid uuid, technologydataname character varying, technologyuuid uuid, licensefee bigint, productcode integer, technologydatadescription character varying, technologydatathumbnail bytea, technologydataimgref character varying, backgroundcolor character varying, taglist uuid[], componentlist uuid[])
    LANGUAGE 'plpgsql'
