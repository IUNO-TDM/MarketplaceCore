...
-- 	1) Why is this Patch necessary?
--  Due to the change on the role and permissions concept it is necessary to update the checkpermission function.
-- 	2) Which Git Issue Number is this patch solving?
--  # 165
-- 	3) Which changes are going to be done?
--  Update CheckPermissions call in all functions.
--: Run Patches
------------------------------------------------------------------------------------------------
--##############################################################################################
...
$$
	DECLARE
		PatchName varchar		 	 := 'iuno_marketplacecore_V0031V_20180313';
		PatchNumber int 		 	 := 0031;
		PatchDescription varchar 	 := 'Update CheckPermissions call in all functions.';
...
$$
		DECLARE
			vPatchNumber int := 0031;
		BEGIN
-- #########################################################################################################################################

-- Function: public.createattribute(character varying, uuid, text[])

-- DROP FUNCTION public.createattribute(character varying, uuid, text[]);
...
$BODY$
	...
	IF(vIsAllowed) THEN
		IF (vCheckOwnership = false) THEN vuseruuid = null; END IF;

				 update technologydata set deleted = true
				 where technologydatauuid = vTechnologyDataUUID and
				 -- Owner or --
				 (vuseruuid is null or createdby = vuseruuid);
...
$BODY$
  LANGUAGE plpgsql;
---#############################################################################################################################################
CREATE OR REPLACE FUNCTION public.getnewproductcode(
    vuseruuid uuid,
    vroles text[])
...
CREATE OR REPLACE FUNCTION public.gettechnologydatabyofferrequest(
    ...
    IN vuseruuid uuid,
    IN vroles text[])
  RETURNS TABLE(technologydatauuid uuid, technologyuuid uuid, technologydataname character varying, technologydata character varying, technologydatadescription character varying, licensefee bigint, technologydatathumbnail bytea, technologydataimgref character varying, backgroundcolor character varying, createdat timestamp with time zone, createdby uuid, updatedat timestamp with time zone, useruuid uuid) AS
$BODY$
